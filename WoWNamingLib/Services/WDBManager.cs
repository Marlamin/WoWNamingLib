...
namespace WoWNamingLib.Services
{
    public class WDBManager
    {
        ...
        private static DBBuild GetBuildInfoFromDB(uint build)
        {
            ...
            dbBuild.version = Namer.build;

            var splitBuild = dbBuild.version.Split('.');

            dbBuild.expansion = short.Parse(splitBuild[0]);
            dbBuild.major = short.Parse(splitBuild[1]);
            dbBuild.minor = short.Parse(splitBuild[2]);
            dbBuild.build = int.Parse(splitBuild[3]);
            return dbBuild;
        }

        public static wdbCache LoadWDB(string path)
        {
